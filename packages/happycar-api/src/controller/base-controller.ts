...
export abstract class BaseController<R> {
  ...
  protected abstract executeImpl(): Promise<
    Response<R> | Response<{ message: ValidationErrorItem[] } | Response<number>>
    >;
  ...
  protected validationError(validationError: ValidationError): Response<any> {
    return this.jsonResponse<{ message: ValidationErrorItem[] }>(422, {
      message: validationError.details || 'parameter required',
    });
  }
